...
![The Whole Setup](./Images/v1/the-whole-setup.png)
...
* **Routing Verification:**
    * ![Backbone OSPF](./Images/v1/area0-backbone-router-interfaces-and-the-routing-table.png)
    * ![Area 1 Core](./Images/v1/area1-core-switch-interfaces-and-the-routing-table.png)
...
* **Link Aggregation (EtherChannel):** Configured dual EtherChannels (LACP) from every core switch to the backbone, and from the core to the access layer.
    * ![Area 1 EtherChannel Config](./Images/v2-part1/area1-c1-etherchannel-config.png)
* **Deterministic Traffic Engineering:** * **RSTP Load Balancing:** Migrated to Rapid PVST+. Configured Core 1 as the Root Bridge for VLANs 10, 20, 30, and Core 2 as the Root for VLANs 40, 50, 60.
    * ![STP Load Balancing](./Images/v2-part2/area1-c1-stp-loadbalancnig-vlan10-30.png)
    * **Gateway Redundancy (HSRP):** Aligned HSRP Active/Standby roles with STP Roots to ensure optimal routing.
    * ![HSRP Config](./Images/v2-part1/area1-c1-hsrp-config.png)
* **Infrastructure Management:**
    * Expanded centralized DHCP and implemented an automated **TFTP Backup** system.
    * ![DHCP Leases](./Images/v2-part2/DHCP-leases.png)

### 📊 Failover Verification
Continuous ping tests confirmed sub-second convergence during simulated core switch failures.
* ![Failover Test](./Images/v2-part1/area0-main-lyswitch-off-conectivity-test.png)
* ![Inter-Department Connectivity](./Images/v2-part2/conectivity-between-differnt-departments-over-the-network.png)
...
* **Secure Administration:** Enforced **SSH** for remote management, completely disabling Telnet.
    * ![Users and Privileges](./Images/v3/users-and-thier-privilges.png)
    * ![SSH Support Login](./Images/v3/ssh-using-support-user.png)
...
* Configured **Extended ACLs** tailored per department. IT maintains full management access, while other departments are strictly limited to necessary internal and internet traffic. Server SSH access is restricted exclusively to the IT subnet.
    * ![Departmental ACLs](./Images/v3/acls-sample-HR-IT.png)
...
* Configured an Edge Router featuring **NAT Overload (PAT)** for general user internet access and **Static NAT** for the secure Server Farm.
    * ![Edge Router NAT Translation](./Images/v3/nat-transilation-on-the-edge-router.png)

---

## 📁 Repository Structure
* `/Configs`: Contains all raw `.txt` configuration files for Switches, Routers, and Servers across all versions.
* `/Images`: Contains architectural diagrams and operational verification screenshots categorized by version.
